...
|<p>-- Palabras del banco personal con su nivel, ordenadas por peso descendente</p><p>SELECT</p><p>`  `w.id, w.word\_en, w.phonetic,</p><p>`  `COALESCE(uwp.level, 0)          AS level,</p><p>`  `COALESCE(uwp.correct\_streak, 0) AS correct\_streak,</p><p>`  `POWER(2, 5 - COALESCE(uwp.level, 0)) AS weight</p><p>FROM words w</p><p>LEFT JOIN user\_word\_progress uwp</p><p>`  `ON uwp.word\_id = w.id AND uwp.user\_id = auth.uid()</p><p>WHERE w.owner\_id = auth.uid()</p><p>`  `AND w.status IN ('active', 'incomplete')</p><p>ORDER BY weight DESC;</p>|
| :- |

> **Nota:** En el banco de palabras (WordBank), la query del hook `useWords` usa `.in('status', ['active', 'incomplete'])` para mostrar tanto palabras activas como incompletas. Las palabras incompletas no tienen progreso y se muestran con badge "Incompleta" y sin nivel/progreso en la UI.
...
|<p>// lib/importWords.js</p><p>// Dependencia: npm install xlsx</p><p>import \* as XLSX from 'xlsx';</p><p></p><p>// 1. Leer archivo y convertir a JSON</p><p>export function parseImportFile(file) {</p><p>`  `return new Promise((resolve) => {</p><p>`    `const reader = new FileReader();</p><p>`    `reader.onload = (e) => {</p><p>`      `const wb = XLSX.read(e.target.result, { type: 'binary' });</p><p>`      `const ws = wb.Sheets[wb.SheetNames[0]];</p><p>`      `const rows = XLSX.utils.sheet\_to\_json(ws, { defval: '' });</p><p>`      `resolve(rows);</p><p>`    `};</p><p>`    `reader.readAsBinaryString(file);</p><p>`  `});</p><p>}</p><p></p><p>// 2. Función auxiliar para normalizar examples (soporta ambos formatos)</p><p>function parseExamples(value) {</p><p>`  `if (!value) return [];</p><p>`  `const trimmed = String(value).trim();</p><p>`  `if (!trimmed) return [];</p><p>`  `try {</p><p>`    `const parsed = JSON.parse(trimmed);</p><p>`    `if (Array.isArray(parsed)) return parsed.filter(e => typeof e === 'string' && e.trim());</p><p>`    `return [trimmed];</p><p>`  `} catch {</p><p>`    `return [trimmed];</p><p>`  `}</p><p>}</p><p></p><p>// 3. Agrupar filas por word\_en (ejemplos se almacenan como arrays)</p><p>export function groupByWord(rows) {</p><p>`  `const map = {};</p><p>`  `let skippedRows = 0;</p><p>`  `for (const row of rows) {</p><p>`    `const key = row.word\_en?.trim().toLowerCase();</p><p>`    `if (!key || !row.translation\_es?.trim()) {</p><p>`      `skippedRows++;</p><p>`      `continue; // omitir filas inválidas</p><p>`    `}</p><p>`    `if (!map[key]) map[key] = { word\_en: row.word\_en.trim(),</p><p>`      `phonetic: row.phonetic?.trim() || null, translations: [] };</p><p>`    `map[key].translations.push({</p><p>`      `translation\_es: row.translation\_es.trim(),</p><p>`      `examples\_en: parseExamples(row.example\_en || row.examples\_en),</p><p>`      `examples\_es: parseExamples(row.example\_es || row.examples\_es),</p><p>`      `explanation:    row.explanation?.trim()   || null,</p><p>`    `});</p><p>`  `}</p><p>`  `return { words: Object.values(map), skippedRows, totalRows: rows.length };</p><p>}</p><p></p><p>// 3. Fusionar con banco personal (misma lógica que mergeWords.js)</p><p>// Para cada word del grupo parseado:</p><p>//   Buscar coincidencia en words personal por LOWER(word\_en)</p><p>//   Si no existe → INSERT word + translations + user\_word\_progress(level=0)</p><p>//   Si existe → fusionar traducciones (ver sección 3.5 del System Design)</p>|
| :- |

### Progreso en tiempo real

|<p>// importWords acepta callback onProgress para reportar avance</p><p>export async function importWords(userId, groupedWords, supabaseClient = supabase, onProgress = null) {</p><p>`  `const total = groupedWords.words.length;</p><p>`  `for (let i = 0; i < groupedWords.words.length; i++) {</p><p>`    `const word = groupedWords.words[i];</p><p>`    `if (onProgress) {</p><p>`      `onProgress({ current: i + 1, total, word: word.word_en });</p><p>`    `}</p><p>`    `// ... procesamiento de cada palabra</p><p>`  `}</p><p>`  `return results;</p><p>}</p><p></p><p>// El callback recibe: { current: number, total: number, word: string }</p><p>// Se ejecuta después de procesar cada palabra</p>|
| :- |

## **4.11 Plantilla Excel descargable y exportación**

|<p>// Generar y descargar plantilla desde el cliente</p><p>import \* as XLSX from 'xlsx';</p><p></p><p>export function downloadTemplate() {</p><p>`  `const headers = [</p><p>`    `'word\_en', 'phonetic', 'translation\_es',</p><p>`    `'example\_en', 'example\_es', 'explanation'</p><p>`  `];</p><p>`  `const examples = [</p><p>`    `['run',  '/rʌn/', 'correr',   'She runs every morning.',</p><p>`     `'Ella corre cada mañana.', 'Uso físico, movimiento.'],</p><p>`    `['run',  '/rʌn/', 'ejecutar', 'Run the program again.',</p><p>`     `'Ejecuta el programa de nuevo.', 'Uso técnico.'],</p><p>`    `['bold', '',      'audaz',    'He made a bold decision.',</p><p>`     `'Tomó una decisión audaz.', 'Describe valentía o atrevimiento.'],</p><p>`  `];</p><p>`  `const ws = XLSX.utils.aoa\_to\_sheet([headers, ...examples]);</p><p>`  `const wb = XLSX.utils.book\_new();</p><p>`  `XLSX.utils.book\_append\_sheet(wb, ws, 'Palabras');</p><p>`  `XLSX.writeFile(wb, 'plantilla\_palabras.xlsx');</p><p>}</p><p></p><p>// Exportar banco personal a Excel</p><p>function formatExamples(examples) {</p><p>`  `if (!examples || !Array.isArray(examples) || examples.length === 0) return '';</p><p>`  `if (examples.length === 1) return examples[0];</p><p>`  `return JSON.stringify(examples);</p><p>}</p><p></p><p>export async function downloadExport(userId) {</p><p>`  `const { data, error } = await supabase</p><p>`    `.from('words')</p><p>`    `.select('\*, word\_translations(\*)')</p><p>`    `.eq('owner\_id', userId)</p><p>`    `.eq('status', 'active');</p><p>`  `if (error) throw error;</p><p>`  `const rows = [];</p><p>`  `for (const word of data) {</p><p>`    `for (const t of word.word\_translations) {</p><p>`      `rows.push({</p><p>`        `word\_en: word.word\_en,</p><p>`        `phonetic: word.phonetic || '',</p><p>`        `translation\_es: t.translation\_es,</p><p>`        `example\_en: formatExamples(t.example\_en),</p><p>`        `example\_es: formatExamples(t.example\_es),</p><p>`        `explanation: t.explanation || '',</p><p>`      `});</p><p>`    `}</p><p>`  `}</p><p>`  `const ws = XLSX.utils.json\_to\_sheet(rows, { header: ['word\_en', 'phonetic', 'translation\_es', 'example\_en', 'example\_es', 'explanation'] });</p><p>`  `const wb = XLSX.utils.book\_new();</p><p>`  `XLSX.utils.book\_append\_sheet(wb, ws, 'Palabras');</p><p>`  `XLSX.writeFile(wb, 'mis\_palabras.xlsx');</p><p>}</p><p></p><p>// Columnas obligatorias: word\_en, translation\_es</p><p>// Columnas opcionales:   phonetic, example\_en, example\_es, explanation</p>|
| :- |
...
## **4.13 Modales de progreso**

|<p>// src/components/ImportProgressModal.jsx</p><p>// Modal con progreso de importación en tiempo real</p><p>// Props: current, total, word, onCancel (opcional)</p><p>// Backdrop: bg-black/50 + backdrop-blur-sm</p>|
| :- |

|<p>// src/components/ProgressModal.jsx</p><p>// Modal genérico de progreso reutilizable</p><p>// Props: message</p><p>// Backdrop: bg-black/50 + backdrop-blur-sm</p><p>// Usado en: WordBank para eliminar palabras</p>|
| :- |
...
## **6.5 Palabras incompletas (borradores)**

|Funcionalidad de "Guardar borrador" en AddWord.jsx:<p></p><p>— Botón "Guardar borrador" visible solo en Step 1 del wizard</p><p>— Valida solo word_en (no requiere traducciones)</p><p>— Llama a `addWord({ word_en, phonetic, translations: [] })`</p><p>— El hook `useWords.addWord()` siempre usa `status` por defecto 'active'</p><p>— La palabra se guarda sin traducciones asociadas</p><p>— Tras guardar, navega a /word-bank con `{ state: { draftSaved: true } }`</p><p>— En WordBank, se muestra un alert ámbar: "Borrador guardado" con mensaje recordatorio</p><p>  El alert desaparece a los 5 segundos o al hacer clic en ✕</p><p></p><p>Detección de incompletitud (sin cambio de schema DB):</p><p>— No se usa un status especial en la base de datos</p><p>— La incompletitud se determina en el cliente:</p><p>  `const isIncomplete = word.status === 'active' && word.word_translations?.length === 0`</p><p>— La columna `words.status` mantiene su CHECK constraint original:</p><p>  `CHECK (status IN ('active', 'archived'))`</p><p></p><p>WordCard.jsx:</p><p>— Si `isIncomplete`: oculta badge de nivel, barra de progreso y traducciones</p><p>— Muestra badge ámbar "Incompleta"</p><p></p><p>Exclusión de práctica (usePractice.js):</p><p>— `fetchWordsForPractice()` filtra palabras sin traducciones:</p><p>  `.filter(word => word.word_translations?.length > 0)`</p><p>— Esto excluye los borradores sin afectar la query a Supabase</p><p></p><p>Consideraciones:</p><p>— Las palabras incompletas se excluyen automáticamente de práctica, exportación a grupo,</p><p>  archivado, exportación Excel y dashboard por los filtros existentes de status = 'active'</p><p>  (o por no tener traducciones en el caso de práctica)</p><p>— Al editar y añadir al menos una traducción en EditWord, la palabra pasa a ser</p><p>  "completa" automáticamente (sin necesidad de cambiar status)</p>|

## **6.6 Lógica de spaced repetition — lib/spacedRepetition.js**
